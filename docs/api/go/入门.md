...
Go 的版本应该>=1.17, 并且需要 Go 模块。

## 安装

使用   `go get` 命令来安装 SDK，并运行以下命令:
...
使用  `ZkSync Provider`, `EthereumProvider` 和 `Wallet`,可以通过 zkSync网络 执行所有基本操作。

::: 警告

⚠️ 切勿将私钥提交给文件追踪历史记录，否则您的账户可能会被泄露。
